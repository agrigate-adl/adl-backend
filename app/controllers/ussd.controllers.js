...
menu.state("paymentMethod", {
  run: () => {
    ...
    const change = Number(
      foi.packages[selectedPackage].totalDue -
        foi.packages[selectedPackage].balance
    );
    menu.con(
...
menu.state("endScratch", {
  run: async () => {
    ...
    if (doc) {
      let newBal =
        Number(foi.packages[selectedPackage].balance) + Number(doc.amount);
      ...
      let updatedPack = await Packages.findByIdAndUpdate(
        foi.packages[selectedPackage].packageID,
        packUpdate,
        {
          new: true,
          useFindAndModify: false,
